...
layout(std430, binding = 4) buffer palettesLayout
{
    uint palette_SSBO[];
};
...
{   
    uint R = (col / (256 * 256 * 256));//gets red hue
...
{
	return DeCompressCol(palette_SSBO[uint(value)+256*(uint(IDX))]);
	//return DeCompressCol(0xff0000ff);
	//return vec4(1,0,0,1);
}
...
vec4 renderPass(vec3 rayDir, vec3 rayPos,vec4 oldColour)
{
    ...
    for(int i = 0; i < voxelNUM; i++){
        ...
        if ((param.x < HDistance.x && param.x > 0.0) || (0>=param.x && 0<param.y) ){
            ...
            if (param.x < HDistance.x && param.x > 0.0){
                HDistance.x = param.x;
                rayProp = palette(param.y,voxel_SSBO[15+(i*16)]);     //the palette function should return a vec4
                temp = normal;
            }
        }
        normal = temp;
